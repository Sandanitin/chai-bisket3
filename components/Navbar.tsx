...
const Navbar = ({ cartCount }: NavbarProps) => {
  ...
  return (
    <nav
      ref={navRef}
      className={`sticky top-0 z-[9999] w-full border-b border-[#2d1a11] transition-all duration-300 ${
        isScrolled
    ...
    >
      <div className="w-full flex h-auto items-center justify-between px-4 md:px-8 lg:px-12">
        ...
        <Link href="/" className="flex items-center space-x-3">
          ...
          <div>
            <span className="text-lg font-semibold text-[#f5eddc] block leading-tight">
              CHAI BISKET
            </span>
            <span className="text-xs font-serif italic tracking-widest text-[#f0a35c] block text-right">
              Indian Eatery
            </span>
          </div>
        </Link>
        ...
        <div className="flex items-center gap-4 ml-auto">
          ...
          <div className="hidden md:flex items-center space-x-6">
            <Link
              href="#menu"
              className="text-[#f5eddc]/80 hover:text-[#ffd9a0] transition-colors font-medium"
            >
              Menu
            </Link>
            <Link
      ...
      {isMenuOpen && (
        <div className="md:hidden absolute top-full left-0 right-0 bg-[#120a07] border-b border-[#2d1a11] shadow-lg z-[100] rounded-b-3xl">
          <div className="container px-4 py-3 flex flex-col space-y-3">
            <a
              href="#menu"
              className="block py-3 text-[#f5eddc] border-b border-[#2d1a11]"
